...
export function GenerateImageDialogContent({
  ...
  const { imageSource, imageModel } = usePresentationState();
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      if (imageSource === "stock") {
        const stockResult = await getImageFromUnsplash(prompt);
        if (stockResult.success && stockResult.imageUrl) {
          imageUrl = stockResult.imageUrl;
        } else {
          throw new Error(stockResult.error ?? "Failed to get stock image");
        }
      } else {
