...
export function GenerateControls({
  ...
  /** Shared helper: fetch image via stock or AI based on current imageSource */
  const fetchImage = async (prompt: string) => {
    if (imageSource === "stock") {
      ...
      if (unsplashResult.success && unsplashResult.imageUrl) {
        return { success: true as const, image: { url: unsplashResult.imageUrl } };
      }
      return { success: false as const, error: unsplashResult.error ?? "Failed to get stock image" };
    }
    // AI generation
    return generateImageAction(prompt, imageModel as ImageModelList);
  };
  ...
  const handleGenerateClick = async () => {
    ...
    setLocalError(null);
    try {
      const result = await fetchImage(newPrompt);
      if (result.success && "image" in result && result.image) {
        ...
        setLocalError((result as { error?: string }).error ?? "Failed to generate image");
      }
    } catch (error) {
      setLocalError(
  ...
  const handleRegenerateClick = async () => {
    ...
    setLocalError(null);
    try {
      const result = await fetchImage(element.query);
      if (result.success && "image" in result && result.image) {
        const { slides, setSlides } = usePresentationState.getState();
        if (isRootImage) {
          setSlides(
        ...
        setLocalError((result as { error?: string }).error ?? "Failed to regenerate image");
      }
    } catch (error) {
      setLocalError(
